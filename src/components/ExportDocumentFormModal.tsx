...
const ExportDocumentFormModal: React.FC<ExportDocumentFormModalProps> = ({ isOpen, onClose, doc }) => {
    ...
    useEffect(() => {
        if (doc) {
            setFormData({
                ...
                // Ensure optional fields are handled if undefined in old records
                notifyPartyAddress: doc.notifyPartyAddress || '',
                notifyPartyEmail: doc.notifyPartyEmail || '',
                notifyPartyPhone: doc.notifyPartyPhone || '',
                debtorAddress: doc.debtorAddress || '',
                debtorEmail: doc.debtorEmail || '',
                debtorPhone: doc.debtorPhone || '',
            });
        } else {
    ...
    const containerTotals = useMemo(() => {
     ...
     const validate = useCallback(() => {
        ...
        if (!formData.debtor) newErrors.debtor = t('validationRequired');
        if (!formData.notifyParty) newErrors.notifyParty = t('validationRequired');
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <Card title={t('clientDetails')}>
                            <div className="space-y-3">
                                <Input label={t('debtor')} value={formData.debtor} onChange={e => handleChange('debtor', e.target.value)} error={errors.debtor} placeholder="Company Name" />
                                <Input label={t('address')} value={formData.debtorAddress} onChange={e => handleChange('debtorAddress', e.target.value)} placeholder="Full Address" />
                                <div className="grid grid-cols-2 gap-3">
                                    <Input label={t('phone')} value={formData.debtorPhone} onChange={e => handleChange('debtorPhone', e.target.value)} />
                                    <Input label={t('email')} value={formData.debtorEmail} onChange={e => handleChange('debtorEmail', e.target.value)} />
                                </div>
                            </div>
                        </Card>
                        
                        <Card title={t('notifyPartyDetails')}>
                            <div className="space-y-3">
                                <Input label={t('notifyParty')} value={formData.notifyParty} onChange={e => handleChange('notifyParty', e.target.value)} error={errors.notifyParty} placeholder="Company Name" />
                                <Input label={t('address')} value={formData.notifyPartyAddress} onChange={e => handleChange('notifyPartyAddress', e.target.value)} placeholder="Full Address" />
                                <div className="grid grid-cols-2 gap-3">
                                    <Input label={t('phone')} value={formData.notifyPartyPhone} onChange={e => handleChange('notifyPartyPhone', e.target.value)} />
                                    <Input label={t('email')} value={formData.notifyPartyEmail} onChange={e => handleChange('notifyPartyEmail', e.target.value)} />
                                </div>
                                <div className="grid grid-cols-2 gap-3">
