...
import type { CultivationCycle, CuttingOperation } from '../types';
...
const PlantingFormModal: React.FC<PlantingFormModalProps> = ({ isOpen, onClose, operationToEdit }) => {
  ...
  const getInitialState = useCallback(() => {
    if (operationToEdit) {
        ...
        return {
          ...
          moduleId: module?.id || '',
          plannedDuration: String(cycle?.plannedDurationDays || CYCLE_DURATION_DAYS),
        };
    }
    ...
    return {
      ...
      moduleId: '',
      plannedDuration: String(CYCLE_DURATION_DAYS),
    };
  ...
  // Calculate Projected Harvest Date whenever planting date or duration changes
  useEffect(() => {
      const days = parseInt(formData.plannedDuration, 10);
      if (formData.plantingDate && !isNaN(days)) {
          const date = new Date(formData.plantingDate);
          date.setDate(date.getDate() + days);
          setProjectedDate(date.toISOString().split('T')[0]);
      } else {
          setProjectedDate('');
      }
  }, [formData.plantingDate, formData.plannedDuration]);
  ...
  const validate = useCallback(() => {
    ...
    if (!formData.plannedDuration || parseInt(formData.plannedDuration) <= 0) newErrors.plannedDuration = t('validationPositiveNumber');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (operationToEdit) {
        ...
        const cycleData: Omit<CultivationCycle, 'id'> = {
            ...
            plannedDurationDays: parseInt(formData.plannedDuration, 10),
            projectedHarvestDate: projectedDate
        };
  ...
  return (
        <Modal isOpen={isOpen} onClose={onClose} title={operationToEdit ? t('editCuttingOperation') : t('newPlanting')} widthClass="max-w-4xl">
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t pt-4 bg-gray-50 dark:bg-gray-800/50 p-3 rounded-lg">
                     <div className="grid grid-cols-2 gap-4">
                        <Input label={t('plannedDuration')} type="number" value={formData.plannedDuration} onChange={e => setFormData(p => ({...p, plannedDuration: e.target.value}))} error={errors.plannedDuration} required />
                        <Input label={t('projectedHarvestDate')} type="date" value={projectedDate} disabled />
                     </div>
                </div>
