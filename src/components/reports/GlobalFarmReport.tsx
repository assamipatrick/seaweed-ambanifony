...
type PivotMetrics = {
    ...
    maxSeverity?: string; // Added to track highest severity for display
    weather: {
...
const GlobalFarmReport: FC<{ month: number; year: number }> = ({ month, year }) => {
    ...
    const data = useData();
    const { sites, seaweedTypes, cultivationCycles, modules, incidents, periodicTests, farmers, stockMovements, pressedStockMovements, exportDocuments, creditTypes, farmerCredits, repayments, employees, roles, monthlyPayments, incidentTypes, incidentSeverities } = data;
    ...
    const { pivotData, sortedIncidentTypes, displaySites, displaySeaweedTypes, onSiteStockReportData, pressedWarehouseReportData, monthlyProductionData, sortedSeverities } = useMemo(() => {
        ...
        // Helper to track which seaweed types are active per site to map general incidents
        const activeTypesPerSite: Record<string, Set<string>> = {};
        ...
        cultivationCycles.forEach(c => {
            ...
            // Track active types for incident mapping
            if (new Date(c.plantingDate) <= periodInfo.endDate && (!c.harvestDate || new Date(c.harvestDate) >= periodInfo.startDate)) {
                if (!activeTypesPerSite[sId]) activeTypesPerSite[sId] = new Set();
                activeTypesPerSite[sId].add(stId);
            }
            ...
            if (incDate >= periodInfo.startDate && incDate <= periodInfo.endDate) {
                 ...
                 if (inc.affectedModuleIds && inc.affectedModuleIds.length > 0) {
                     inc.affectedModuleIds.forEach(modId => {
                         ...
                         // If no active cycle found, try finding recent
                         if (!cycle) {
                     ...
                     // Fallback: If no specific modules selected, assume it affects ALL active types on that site
                     const types = activeTypesPerSite[inc.siteId];
                     if (types) {
                         types.forEach(tId => affectedPairs.add(`${tId}|${inc.siteId}`));
                     }
                 }

                 // Apply incident count and severity to the correct cells
                 affectedPairs.forEach(pair => {
                     const [stId, sId] = pair.split('|');
                     
                     if (structure[stId] && structure[stId][sId]) {
                        const target = structure[stId][sId];
                        target.incidents[inc.type] = (target.incidents[inc.type] || 0) + 1;
                        
                        // Update max severity
                        const currentMax = target.maxSeverity;
                        const currentRank = currentMax ? (severityRank[currentMax] ?? -1) : -1;
                        const newRank = severityRank[inc.severity] ?? 0;

                        if (newRank > currentRank) {
                            target.maxSeverity = inc.severity;
                        }

                        // Also update Total
                        const totalTarget = structure[stId]['TOTAL'];
                        totalTarget.incidents[inc.type] = (totalTarget.incidents[inc.type] || 0) + 1;
                        // Note: We don't propagate maxSeverity to TOTAL usually, but could if needed.
                     }
                 });
            }
         });
        ...
        const sortedSeverities = [...incidentSeverities].sort((a, b) => {
        ...
        return { pivotData: structure, sortedIncidentTypes: incidentTypeNames, displaySites, displaySeaweedTypes, onSiteStockReportData, pressedWarehouseReportData, monthlyProductionData: monthlyProd, page1Data, sortedSeverities };

    }, [cultivationCycles, modules, farmers, incidents, periodicTests, sites, seaweedTypes, periodInfo, stockMovements, pressedStockMovements, year, month, incidentTypes, incidentSeverities, t]);
    ...
    return (
        <>
            <CombinedOverviewPage 
                ...
                sortedSeverities={sortedSeverities}
            ...
            />
            <Page2 period={periodInfo.period} employees={employees} sites={displaySites} roles={roles} page={2} totalPages={TOTAL_PAGES} monthlyPayments={monthlyPayments} month={month} year={year} language={language as 'en' | 'fr'} />
            <Page3 period={periodInfo.period} onSiteData={onSiteStockReportData} warehouseData={pressedWarehouseReportData} sites={displaySites} seaweedTypes={seaweedTypes} page={3} totalPages={TOTAL_PAGES} language={language as 'en' | 'fr'} />
            <Page4 period={periodInfo.period} year={periodInfo.year} exportDocuments={exportDocuments} seaweedTypes={seaweedTypes} page={4} totalPages={TOTAL_PAGES} language={language as 'en' | 'fr'} />
            <Page5 period={periodInfo.period} months={periodInfo.months} monthlyData={monthlyProductionData} year={year} sites={displaySites} seaweedTypes={seaweedTypes} page={5} totalPages={TOTAL_PAGES} language={language as 'en' | 'fr'} />
...
// --- COMBINED PAGE 1 UPDATE ---
const CombinedOverviewPage: FC<any> = ({ period, pivotData, displaySites, displaySeaweedTypes, sortedSeverities, nextMonths, t, page, totalPages, language }) => {
    const { settings } = useSettings();
    const getD = (typeId: string, siteId: string) => pivotData[typeId]?.[siteId];
    
    // Explicit red styling for severities
    const getSeverityClass = (severity?: string): string => {
        if (!severity) return "text-gray-300";
        switch (severity) {
            case 'CRITICAL':
                return "text-red-800 bg-red-200 border border-red-300 font-extrabold"; 
            case 'HIGH':
                return "text-red-700 bg-red-100 border border-red-200 font-bold";
            case 'MEDIUM':
                return "text-orange-600 bg-orange-50 font-semibold"; // Medium is usually orange/yellow
            case 'LOW':
                ...
                <div className="w-full overflow-hidden">
                    <table className="w-full border-collapse mb-2 table-fixed text-[8px]">
                        ...
                        <tbody>
                            ...
                            <tr><Cell colSpan={colCount} header align="left" className="pl-2 bg-gray-200 uppercase text-[9px] tracking-wider border-t-2 border-gray-600 font-bold">{t('apercuGeneralDeLaFerme')}</Cell></tr>
                            ...
                            <tr><Cell colSpan={colCount} className="bg-gray-200 text-black font-bold py-1.5 text-center text-[9px] border-t-2 border-gray-600 tracking-widest">
                                {t('monitoringReport').toUpperCase()}
                            </Cell></tr>
                            
                             {/* INCIDENT STATUS ROW - Shows Red Critical/High alerts */}
                             <tr>
                                <Cell colSpan={colCount} header align="left" className="pl-2 bg-gray-100 uppercase text-[8px] tracking-wider font-bold border-t border-gray-300">{t('incidentsLabel')}</Cell>
                            </tr>
                            {/* Severity PER SITE/TYPE - Displayed in Red for Critical/High */}
                             <tr>
                                <Cell align="left" className="pl-4 font-medium border-r border-gray-300">{t('severity')}</Cell>
                                {displaySeaweedTypes.map((st: any) => (
                                    <React.Fragment key={st.id}>
                                        {displaySites.map((s: any) => {
                                            const sev = getD(st.id, s.id)?.maxSeverity;
                                            return <Cell key={s.id} className={getSeverityClass(sev)}>{sev ? t(`severity_${sev}` as any) : '-'}</Cell>
                                        })}
                                        <Cell bold className="bg-gray-50 border-l border-gray-300 text-gray-400">-</Cell>
                                    </React.Fragment>
                                ))}
                            </tr>
                 ...
                 <NarrativeBox title={t('narratif_explicatif_tableau')} generator={async () => generateGlobalFarmOverviewNarrative({}, language)} />
            </div>
...
const Page2: FC<any> = ({ period, employees, sites, roles, page, totalPages, monthlyPayments, month, year, language }) => {
    const {t} = useLocalization();
    const { settings } = useSettings();
    ...
    return (
        <PrintPage>
             ...
             <div className="flex flex-col gap-2">
                ...
                <div className="w-full">
                    <NarrativeBox title={t('narratif_repartition_salaries')} generator={async () => generateSalariesNarrative({}, language)} />
                </div>
             </div>
             <ReportFooter page={page} totalPages={totalPages} />
        </PrintPage>
...
const Page3: FC<any> = ({ period, onSiteData, warehouseData, sites, seaweedTypes, page, totalPages, language }) => {
    const {t} = useLocalization();
    const { settings } = useSettings();
        ...
        <PrintPage>
            ...
            <div className="flex flex-col gap-6 mt-2">
                <section>
                    ...
                    <NarrativeBox title={t('narratif_stocks_sites')} generator={async () => generateSiteStocksNarrative({}, language)} />
                </section>
                ...
                <section>
                    ...
                    <NarrativeBox title={t('narratif_stocks_magasin')} generator={async () => generateWarehouseStocksNarrative({}, language)} />
                </section>
            </div>
            <ReportFooter page={page} totalPages={totalPages} />
        </PrintPage>
...
const Page4: FC<any> = ({ period, year, exportDocuments, seaweedTypes, page, totalPages, language }) => {
    // ... (unchanged, just ensure language is passed to narrative)
    const { t } = useLocalization();
    const { settings } = useSettings();
    ...
    const periodDocs = useMemo(() => {
        return exportDocuments.filter(d => {
            return d.date.includes(String(year)); 
        });
    ...
    return (
        <PrintPage>
             ...
             <div className="flex-grow flex flex-col gap-4 mt-2 overflow-visible">
                ...
                <NarrativeBox title={t('narratif_exportations_realisees')} generator={async () => generateExportsNarrative({}, language)} />
             </div>
             <ReportFooter page={page} totalPages={totalPages} />
        </PrintPage>
    );
...
const Page5: FC<any> = ({ period, months, monthlyData, year, sites, seaweedTypes, page, totalPages, language }) => {
    // ... (unchanged, just ensure language is passed)
    const {t} = useLocalization();
    const { settings } = useSettings();
    ...
    return (
        <PrintPage>
            ...
            <div className="flex-grow flex flex-col gap-2 mt-2 overflow-visible">
                 ...
                 <NarrativeBox title={t('narratif_production_mensuelle')} generator={async () => generateMonthlyProductionNarrative({}, language)}/>
            </div>
            <ReportFooter page={page} totalPages={totalPages} />
        </PrintPage>
    );
...
const Page6: FC<any> = ({ 
    period, sixMonths, prevMonthName, creditTypes, farmerCredits, repayments, year, month, groupedData, totals, page, totalPages, language 
}) => {
    // ... (unchanged, just ensure language is passed)
    const {t, language: lang } = useLocalization();
    const { settings } = useSettings();
    const monthHeaders = sixMonths.map((d: Date) => {
        const monthName = d.toLocaleString(lang, { month: 'short' }).replace('.', '').toUpperCase();
        const year2digit = d.getFullYear().toString().slice(-2);
        ...
        <PrintPage>
             ...
             <div className="flex-grow flex flex-col gap-4 mt-2 overflow-visible">
                <section className="flex flex-col">
                    ...
                    <NarrativeBox title={t('narratif_situation_credits')} generator={async () => generateCreditsSituationNarrative(creditReportData, language)} />
                </section>
                ...
                <section className="flex flex-col mt-4">
                     ...
                     <NarrativeBox title={t('narratif_statistique_individuelle')} generator={async () => generateIndividualFarmerStatsNarrative(groupedData, language)} />
                </section>
             </div>
             <ReportFooter page={page} totalPages={totalPages} />
        </PrintPage>
