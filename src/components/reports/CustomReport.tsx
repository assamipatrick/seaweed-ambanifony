...
const CustomReport: FC<CustomReportProps> = ({ month, year, sections }) => {
    ...
    const { 
        cultivationCycles, modules, sites, seaweedTypes, farmerCredits, farmers, creditTypes,
    ...
    const inventoryData = useMemo(() => {
        ...
        // FIX: Explicitly type the Map to avoid 'unknown' type inference on values
        const seaweedTypeMap = new Map<string, { name: string, dryPrice: number }>(seaweedTypes.map(st => [st.id, { name: st.name, dryPrice: st.dryPrice }]));
        ...
        const data = Array.from(stockMap.entries())
            .map(([key, stock]) => {
                ...
                // FIX: Check if seaweedTypeInfo is defined before accessing properties
