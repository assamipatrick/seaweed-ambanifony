...
export const CultivationCycleList: React.FC<CultivationCycleListProps> = ({ initialFilters, pageTitle }) => {
    ...
    const [harvestDetails, setHarvestDetails] = useState({ date: '', notes: '', linesHarvested: '', harvestedWeight: '', cuttingsWeight: '', cuttingsIntendedUse: '' });
    ...
    useEffect(() => {
        if (cycle) {
            ...
            setFormData({
                ...
                status: cycle.status,
                linesPlanted: String(cycle.linesPlanted || module?.lines || '')
            });
        }
    }, [cycle, modules]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (cycle) {
            onSave({
                ...
                linesPlanted: parseFloat(formData.linesPlanted)
            });
        }
    };
        ...
        <Modal isOpen={isOpen} onClose={onClose} title={t('editCycle')}>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <Input label={t('linesPlanted')} type="number" value={formData.linesPlanted} onChange={e => handleChange('linesPlanted', e.target.value)} required />
                </div>
